...
결과물:
    dist/pickiclass-recorder-<target>/     flet build 번들(설치기의 원본)
...
# Finder에 "Pickiclass Recorder.app"로 보일 수 있다(패키징 파이프라인의 제약이며, 인앱 표시나
# 창 제목 등은 여전히 한글 _PRODUCT를 그대로 쓴다). vpk가 이 버그를 고치면 _PRODUCT로 되돌릴 것.
_PACK_TITLE = "Pickiclass Recorder"
...
# project.name(="pickiclass-recorder")을 그대로 Windows 실행 파일 OUTPUT_NAME으로 쓴다.
APP_EXE_WINDOWS = "pickiclass-recorder.exe"
...
def main() -> int:
    ...
    # pyproject.toml(SSOT)의 버전을 pickiclass/__init__.py에 반영한 뒤(flet build가 이 파일을
